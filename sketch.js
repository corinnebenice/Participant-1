...
function draw() { //code that runs constantly
  ...
  if (begin == true) {
    ...
    if ((time - lasttime3) > 100) { //timer every 0.1 sec
      lasttime3 = time;
      if (start == true) {
        soundtimer++;
        pausetimer = 0;
    ...
    if (noisetimer > 50) { //when surrounding noise longer than x there is a true interruption
      interruption = true;
    } else {
      interruption = false;
    }
    ...
    if (sensortimer > 20) { //when interruption longer than 2 sec, then notification will be shown
      // fill(0, 255, 0);
    ...
    console.log(vol); //to control a variable in the bottom of screen
